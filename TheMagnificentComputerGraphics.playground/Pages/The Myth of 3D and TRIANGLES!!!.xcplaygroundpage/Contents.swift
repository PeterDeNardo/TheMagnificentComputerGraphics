 ...
 * And see the importance of TRIANGLES ⚠️🔺!!!
 ...
 And about triangles, if Vertex is our base, they are our material, all the shapes that we want to build use triangles as a base, for example, click on the Shape button to change the figure on the screen, notice that the faces of the Cube are constructed with 2 triangles. In resume TRIANGLES ARE IMPORTANT and always when we are connecting the vetices, we have to think about how to form the desired shape using trianglos, which is what we do in the indexArray
